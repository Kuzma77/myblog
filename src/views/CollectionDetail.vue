		...
		<div class="row">
					...
					<div v-for="(item, index) in topicVo.articleList" :key="index" class="col-12 border-bottom">
						<div class="media-wraaper">
							<div class="media-left">
								<router-link :to="{ path: '/user/' + item.author.id }">
			...
			<div class="col-4">
				...
				<div class="box border-bottom">
					...
					<router-link :to="{ path: '/user/' + topicVo.admin.id }">
					<img :src="topicVo.admin.avatar" class="avatar-xs" />
					</router-link>
					<p>{{ topicVo.admin.nickname }}</p>
				</div>
				<div class="box border-bottom"><p class="sub-title">关注的人</p></div>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	created() {
		...
		this.axios.get('http://localhost:8080/api/topic/' + id).then(res => {
